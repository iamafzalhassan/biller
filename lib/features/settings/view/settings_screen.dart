...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  late final ProfileFields _fields;
  ...
  Future<void> _save() async {
    SoftKeyboard.dismiss();
    final List<String> phones = _fields.phones;
    if (phones.isEmpty || phones.any((String phone) => !Validators.isValidPhone(phone))) {
      context.showErrorSnack('Enter at least one phone number, each 10 digits starting with 0. Nothing has been saved yet.');
      return;
    }
    if (!Validators.isValidEmail(_fields.emailController.text)) {
      context.showErrorSnack('Enter a valid email address, like name@example.com. Nothing has been saved yet.');
      return;
    }
    if (_fields.prefixController.text.trim().isEmpty || !Validators.isValidDeviceId(_fields.deviceIdController.text.trim().toUpperCase())) {
    ...
    final BusinessProfile profile = _fields.toProfile(logoPath: _logoPath, terms: _terms);
  ...
  Widget _retentionField() => AppTextField(
  ...
  Widget _form() => ListView(
    ...
    children: <Widget>[
      _inset(<Widget>[
        ...
        _field(controller: _fields.nameController, focusNode: _fields.nameFocus, label: 'Business Name', nextFocus: _fields.phone1Focus),
        const SizedBox(height: AppSpacing.md),
        _phoneField(_fields.phone1Controller, _fields.phone1Focus, 'Phone 1', nextFocus: _fields.phone2Focus),
        const SizedBox(height: AppSpacing.md),
        _phoneField(_fields.phone2Controller, _fields.phone2Focus, 'Phone 2 (Optional)', nextFocus: _fields.phone3Focus),
        const SizedBox(height: AppSpacing.md),
        _phoneField(_fields.phone3Controller, _fields.phone3Focus, 'Phone 3 (Optional)', nextFocus: _fields.emailFocus),
        const SizedBox(height: AppSpacing.md),
        _field(controller: _fields.emailController, focusNode: _fields.emailFocus, isUpperCase: false, keyboardType: TextInputType.emailAddress, label: 'Email', nextFocus: _fields.noFocus),
        const SizedBox(height: AppSpacing.xl),
        const SectionHeader(label: 'ADDRESS'),
        _field(controller: _fields.noController, focusNode: _fields.noFocus, label: 'No', nextFocus: _fields.streetFocus),
        const SizedBox(height: AppSpacing.md),
      ...
      _inset(<Widget>[
        ...
        NumberingSection(
          deviceIdController: _fields.deviceIdController,
          deviceIdFocus: _fields.deviceIdFocus,
          nextInvoiceNumber: _fields.invoiceNumber(ref.read(settingsRepositoryProvider).nextSequence),
          prefixController: _fields.prefixController,
          prefixFocus: _fields.prefixFocus,
          onChanged: _refresh,
